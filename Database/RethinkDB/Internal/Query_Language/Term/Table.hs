...
module Database.RethinkDB.Internal.Query_Language.Term.Table (Table(..)) where
import Prelude ((+), (/))
...
data Table = Table{table_ref :: !Query_Language.TableRef}
           deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable Table where
  mergeAppend (Table x'1) (Table y'1) = Table (P'.mergeAppend x'1 y'1)
 
instance P'.Default Table where
  defaultValue = Table P'.defaultValue
 
instance P'.Wire Table where
  wireSize ft' self'@(Table x'1)
   = case ft' of
       10 -> calc'Size
