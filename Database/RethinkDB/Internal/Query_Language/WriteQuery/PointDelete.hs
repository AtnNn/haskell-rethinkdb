...
module Database.RethinkDB.Internal.Query_Language.WriteQuery.PointDelete (PointDelete(..)) where
import Prelude ((+), (/))
...
data PointDelete = PointDelete{table_ref :: !Query_Language.TableRef, attrname :: !P'.Utf8, key :: !Query_Language.Term}
                 deriving (Prelude'.Show, Prelude'.Eq, Prelude'.Ord, Prelude'.Typeable, Prelude'.Data)
 
instance P'.Mergeable PointDelete where
  mergeAppend (PointDelete x'1 x'2 x'3) (PointDelete y'1 y'2 y'3)
   = PointDelete (P'.mergeAppend x'1 y'1) (P'.mergeAppend x'2 y'2) (P'.mergeAppend x'3 y'3)
 
instance P'.Default PointDelete where
  defaultValue = PointDelete P'.defaultValue P'.defaultValue P'.defaultValue
 
instance P'.Wire PointDelete where
  wireSize ft' self'@(PointDelete x'1 x'2 x'3)
   = case ft' of
       10 -> calc'Size
  ...
  wirePut ft' self'@(PointDelete x'1 x'2 x'3)
   = case ft' of
       10 -> put'Fields
       11 -> do
